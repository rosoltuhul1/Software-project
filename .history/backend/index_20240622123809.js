...
app.get('/searchItems', async function(req, res) {
  try {
    const posts = await Post.find({ free: null }).populate('registerID');
    console.log("search");
    const results = posts.map(post => {
      return {
        ...
        image: post.image,
        type: post.type,
        fname: post.registerID.fname,
        lname: post.registerID.lname
      };
    });
...
// routes/posts.js or wherever you define your routes
app.get('/otherposts', async (req, res) => {
  try {
    console.log('others');
    // Fetch posts of type "Others"
    const OtherPosts = await Post.find({ type: "Others" }).select('image description registerID type phoneNumber');
