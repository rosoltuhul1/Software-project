...
class FavouriteTime extends StatefulWidget {
  @override
  _FavouriteTimeState createState() => _FavouriteTimeState();
}

class _FavouriteTimeState extends State<FavouriteTime> {
  CalendarFormat _calendarFormat = CalendarFormat.month;
  DateTime _focusedDay = DateTime.now();
  DateTime? _selectedDay;
  Map<DateTime, List<String>> _selectedTimes = {};

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Favourite Time',
          style: TextStyle(
            color: Colors.white,
            fontSize: 25),
        ),
      ...
      body: Column(
        children: [
          ...
          if (_selectedDay != null && _selectedTimes.containsKey(_selectedDay))
            Expanded(
              child: ListView.builder(
                itemCount: _selectedTimes[_selectedDay]!.length,
                itemBuilder: (context, index) {
                  return ListTile(
                    title: Text(_selectedTimes[_selectedDay]![index]),
                  );
                },
              ),
            ),
  ...
  void _showTimePicker(BuildContext context, DateTime selectedDay) {
    showTimePicker(
      context: context,
      ...
      if (pickedTime != null) {
        ...
        setState(() {
          if (!_selectedTimes.containsKey(selectedDay)) {
            _selectedTimes[selectedDay] = [];
          }
          _selectedTimes[selectedDay]!.add(formattedTime);
        });
      }
    });
  }
