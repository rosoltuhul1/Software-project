...
class _Otherspage extends State<Otherspage> {
  ...
  final response = await http.get(Uri.parse('http://$ipAddress:3000/otherposts'));
    ...
    setState(() {
      Others = data.map((item) {
        return {
          ...
          "phoneNumber": item["phoneNumber"], // Add the phone number field
        };
      }).toList();
    });
  } else {
    throw Exception('Failed to load others photo');
  }
}
